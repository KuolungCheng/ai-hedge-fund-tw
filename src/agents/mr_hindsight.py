...
def _compute_recent_return(prices: list) -> float | None:
    if not prices:
        return None

    closes = []
    for price in prices:
        close_val = getattr(price, "close", None)
        if close_val is None and isinstance(price, dict):
...
def _select_hindsight_signal(
    ...
    if recent_return is not None:
        ...
        if recent_return <= -0.03:
            return "bearish", "price_trend"

    if bullish_signals > bearish_signals:
        return "bullish", "news_consensus"
    ...
    if recent_return is not None:
        ...
        if recent_return <= -0.01:
            return "bearish", "price_tiebreaker"
    return "neutral", "neutral_tie"
...
def news_sentiment_agent(state: AgentState, agent_id: str = "news_sentiment_agent"):
    ...
    for ticker in tickers:
        ...
        trend_start_date = (datetime.datetime.strptime(end_date, "%Y-%m-%d") - datetime.timedelta(days=90)).strftime("%Y-%m-%d")
        ...
        recent_return = _compute_recent_return(price_history)
        ...
        if company_news:
            ...
            if articles_without_sentiment:
              ...
              progress.update_status(agent_id, ticker, f"分析 {len(articles_to_analyze)} 則新聞情緒")
              ...
              for idx, news in enumerate(articles_to_analyze):
                ...
                progress.update_status(agent_id, ticker, f"分析新聞情緒：第 {idx + 1}/{len(articles_to_analyze)} 則")
                prompt = (
        ...
        overall_signal, signal_source = _select_hindsight_signal(
    ...
    if state.get("metadata", {}).get("show_reasoning"):
        show_agent_reasoning(sentiment_analysis, "新聞情緒分析代理")
...
def _calculate_confidence_score(
    ...
    if total_signals == 0:
        ...
        base_confidence = round((max(bullish_signals, bearish_signals) / total_signals) * 100, 2)
