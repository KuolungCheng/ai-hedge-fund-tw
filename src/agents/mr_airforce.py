...
def valuation_analyst_agent(state: AgentState, agent_id: str = "valuation_analyst_agent"):
    ...
    for ticker in tickers:
        progress.update_status(agent_id, ticker, "取得財務資料")
        ...
        # --- Enhanced line‑items ---
        progress.update_status(agent_id, ticker, "蒐集完整財報欄位")
        ...
        if len(line_items) < 2:
            progress.update_status(agent_id, ticker, "失敗：財報欄位不足")
            continue
        ...
        progress.update_status(agent_id, ticker, "計算 WACC 與強化版 DCF")
        ...
        if not market_cap:
            progress.update_status(agent_id, ticker, "失敗：無法取得市值")
            continue
        ...
        if total_weight == 0:
            progress.update_status(agent_id, ticker, "失敗：所有估值方法結果皆為 0")
            continue
