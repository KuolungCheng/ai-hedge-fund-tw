...
def technical_analyst_agent(state: AgentState, agent_id: str = "technical_analyst_agent"):
    ...
    for ticker in tickers:
        progress.update_status(agent_id, ticker, "分析價格資料")
        ...
        if not prices:
            progress.update_status(agent_id, ticker, "失敗：找不到價格資料")
            continue

        # Convert prices to a DataFrame
        prices_df = prices_to_df(prices)

        progress.update_status(agent_id, ticker, "計算趨勢訊號")
        trend_signals = calculate_trend_signals(prices_df)

        progress.update_status(agent_id, ticker, "計算均值回歸")
        mean_reversion_signals = calculate_mean_reversion_signals(prices_df)

        progress.update_status(agent_id, ticker, "計算動能")
        momentum_signals = calculate_momentum_signals(prices_df)

        progress.update_status(agent_id, ticker, "分析波動度")
        volatility_signals = calculate_volatility_signals(prices_df)

        progress.update_status(agent_id, ticker, "統計分析")
        ...
        combined_signal, trend_bias = apply_trend_bias(
        ...
        technical_analysis[ticker] = {
            ...
            "confidence": round(safe_float(combined_signal["confidence"]) * 100),
            "reasoning": {
                "trend_following": {
                    "signal": trend_signals["signal"],
                    "confidence": round(safe_float(trend_signals["confidence"]) * 100),
                    "metrics": normalize_pandas(trend_signals["metrics"]),
                },
                "mean_reversion": {
                    "signal": mean_reversion_signals["signal"],
                    "confidence": round(safe_float(mean_reversion_signals["confidence"]) * 100),
                    "metrics": normalize_pandas(mean_reversion_signals["metrics"]),
                },
                "momentum": {
                    "signal": momentum_signals["signal"],
                    "confidence": round(safe_float(momentum_signals["confidence"]) * 100),
                    "metrics": normalize_pandas(momentum_signals["metrics"]),
                },
                "volatility": {
                    "signal": volatility_signals["signal"],
                    "confidence": round(safe_float(volatility_signals["confidence"]) * 100),
                    "metrics": normalize_pandas(volatility_signals["metrics"]),
                },
                "statistical_arbitrage": {
                    "signal": stat_arb_signals["signal"],
                    "confidence": round(safe_float(stat_arb_signals["confidence"]) * 100),
    ...
    if state["metadata"]["show_reasoning"]:
        show_agent_reasoning(technical_analysis, "技術分析代理")
...
def apply_trend_bias(
    ...
    if prices_df.empty or "close" not in prices_df.columns:
        return combined_signal, {"applied": False, "reason": "no_price_data"}

    ma10 = prices_df["close"].rolling(window=10).mean()
    if ma10.empty or pd.isna(ma10.iloc[-1]):
        return combined_signal, {"applied": False, "reason": "insufficient_ma10_data"}

    close = float(prices_df["close"].iloc[-1])
    ma10_last = float(ma10.iloc[-1])
    above_ma10 = close >= ma10_last
    trend_bullish = trend_signal.get("signal") == "bullish"
    trend_bearish = trend_signal.get("signal") == "bearish"
    momentum_bullish = momentum_signal.get("signal") == "bullish"
    momentum_bearish = momentum_signal.get("signal") == "bearish"

    signal = combined_signal.get("signal", "neutral")
    confidence = float(combined_signal.get("confidence", 0.0))

    if signal == "neutral" and above_ma10 and trend_bullish and not momentum_bearish:
        return {"signal": "bullish", "confidence": max(confidence, 0.55)}, {
            "applied": True,
            "direction": "bullish",
            "reason": "多頭趨勢且站上10日線，避免中立鈍化",
            "close": round(close, 2),
            "ma10": round(ma10_last, 2),
        }

    if signal == "neutral" and (not above_ma10) and trend_bearish and not momentum_bullish:
        return {"signal": "bearish", "confidence": max(confidence, 0.55)}, {
            "applied": True,
            "direction": "bearish",
            "reason": "空頭趨勢且跌破10日線，避免中立鈍化",
            "close": round(close, 2),
            "ma10": round(ma10_last, 2),
        }
